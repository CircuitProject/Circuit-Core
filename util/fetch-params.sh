...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/CIRCUITParams"
    else
        PARAMS_DIR="$HOME/.circuit-params"
    fi
fi
...
Failed to fetch the CIRCUIT zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
CIRCUIT - fetch-params.sh

This script will fetch the CIRCUIT zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common CIRCUIT zkSNARK parameters. Note that it is
