...
The build script we'll be using is contained in the CIRCUIT github repository ([contrib/gitian-build.py](https://github.com/circuit-project/circuit/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the CIRCUIT repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/pandagrows/circuit-crct-coin/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, circuit-detached-sigs, and circuit GitHub repos.
