...
#include "zcrct/zerocoin.h"
#include "zcrct/zcrctmodule.h"
...
#if defined(NDEBUG)
#error "CIRCUIT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ucrct) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zCRCT mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zCRCT mint output in tx %s",
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k CRCT
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZCRCTModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
{
    util::ThreadRename("circuit-scriptch");
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZCRCTModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const auto& txIn : block.vtx) {
        ...
        // double check that there are no double spent zCRCT spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCRCT serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nCircuitBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nCircuitBadBlockBits)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> crctInputs;
        std::vector<CTxIn> zCRCTInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zCRCTInputs.push_back(stakeIn);
            }else{
                crctInputs.push_back(stakeIn);
            }
        }
        const bool hasCRCTInputs = !crctInputs.empty();
        const bool hasZCRCTInputs = !zCRCTInputs.empty();
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zCRCT inputs.
            if(hasZCRCTInputs) {
                for (const CTxIn& zCrctInput : zCRCTInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcrct");
