...
bool CLegacyZCrctStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZCrctStake from non zc-spend", __func__);
    ...
    *this = CLegacyZCrctStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zcrct stake origin", __func__);

    // All good
    return true;
}

CLegacyZCrctStake::CLegacyZCrctStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZCrctStake::GetIndexFrom() const
{
...
CAmount CLegacyZCrctStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZCrctStake::GetUniqueness() const
{
...
bool CLegacyZCrctStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zCRCT stake block: height %d outside range", __func__, nHeight);
