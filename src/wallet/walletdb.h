...
public:
    ...
    bool WriteZCRCTSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZCRCTSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZCRCTSeed_deprecated(uint256& seed);
    bool EraseZCRCTSeed();
    bool EraseZCRCTSeed_deprecated();

    bool WriteZCRCTCount(const uint32_t& nCount);
    bool ReadZCRCTCount(uint32_t& nCount);
