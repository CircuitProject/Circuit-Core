...
#ifndef CIRCUIT_STAKEINPUT_H
#define CIRCUIT_STAKEINPUT_H
...
public:
    ...
    virtual bool IsZCRCT() const = 0;
    virtual CDataStream GetUniqueness() const = 0;
    virtual bool ContextCheck(int nHeight, uint32_t nTime) = 0;
};


class CCrctStake : public CStakeInput
{
private:
...
public:
    CCrctStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CCrctStake* NewCrctStake(const CTxIn& txin);
    ...
    bool IsZCRCT() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //CIRCUIT_STAKEINPUT_H
