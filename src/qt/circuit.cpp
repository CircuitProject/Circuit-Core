...
#include "config/circuit-config.h"
#endif
...
#include "qt/circuit/splash.h"
#include "qt/circuit/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("circuit-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in circuit.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in circuit.qrc)
...
/** Class encapsulating CIRCUIT Core startup and shutdown.
...
/** Main CIRCUIT application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (CIRCUITGUI)
...
{
    window = new CIRCUITGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &CIRCUITGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &CIRCUITGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(CIRCUITGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(CIRCUITGUI::DEFAULT_WALLET);
        ...
        // CIRCUIT: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &CIRCUITGUI::handlePaymentRequest);
        connect(window, &CIRCUITGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. CIRCUIT can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(circuit_locale);
    Q_INIT_RESOURCE(circuit);
    ...
    /// 6. Determine availability of data directory and parse circuit.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("CIRCUIT Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("CIRCUIT Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // circuit: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("CIRCUIT Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
