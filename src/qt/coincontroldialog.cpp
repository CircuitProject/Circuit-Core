...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select CRCT Outputs to Spend" :
            "Select Shielded CRCT to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::CRCT;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show CIRCUIT address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
