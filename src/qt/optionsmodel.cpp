...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::CRCT);
    ...
    if (settings.contains("nAnonymizeCircuitAmount") || reset)
        gArgs.SoftSetArg("-anonymizecircuitamount", settings.value("nAnonymizeCircuitAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in circuit.cpp during shotdown to also remove the window positions
