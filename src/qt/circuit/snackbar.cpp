...
#include "qt/circuit/forms/ui_snackbar.h"
#include "qt/circuit/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(CIRCUITGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &CIRCUITGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
