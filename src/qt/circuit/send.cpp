...
#include "qt/circuit/send.h"
#include "qt/circuit/forms/ui_send.h"
#include "qt/circuit/addnewcontactdialog.h"
#include "qt/circuit/qtutils.h"
#include "qt/circuit/sendchangeaddressdialog.h"
#include "qt/circuit/optionbutton.h"
#include "qt/circuit/sendconfirmdialog.h"
#include "qt/circuit/guitransactionsutils.h"
#include "qt/circuit/loadingdialog.h"
...
SendWidget::SendWidget(CIRCUITGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-crct");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onCRCTSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onCRCTSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent CRCTs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (walletModel && availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield CRCTs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent CRCTs to shield."));
    }
}
...
void SendWidget::onCRCTSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto crctAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(crctAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(crctAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(crctAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
