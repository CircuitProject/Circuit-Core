...
#include "qt/circuit/forms/ui_dashboardwidget.h"
#include "qt/circuit/sendconfirmdialog.h"
#include "qt/circuit/txrow.h"
...
DashboardWidget::DashboardWidget(CIRCUITGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &CIRCUITGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("CRCT")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair CRCT, zCRCT
