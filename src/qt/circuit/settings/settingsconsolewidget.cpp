...
#include "qt/circuit/settings/forms/ui_settingsconsolewidget.h"
...
#include "qt/circuit/settings/moc_settingsconsolewidget.cpp"
...
SettingsConsoleWidget::SettingsConsoleWidget(CIRCUITGUI* _window, QWidget *parent) :
...
void SettingsConsoleWidget::clear(bool clearHistory)
{
    ...
    messageInternal(CMD_REPLY, (tr("Welcome to the CIRCUIT RPC console.") + "<br>" +
...
static bool PotentiallyDangerousCommand(const QString& cmd)
{
    ...
    if (cmd.size() >= 13 && cmd.leftRef(11) == "dumpprivkey") {
        // valid CIRCUIT Transparent Address
        std::vector<std::string> args;
