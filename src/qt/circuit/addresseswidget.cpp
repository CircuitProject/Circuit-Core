...
#include "qt/circuit/forms/ui_addresseswidget.h"
#include "qt/circuit/addresslabelrow.h"
#include "qt/circuit/addnewaddressdialog.h"
#include "qt/circuit/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(CIRCUITGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto crctAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(crctAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(crctAdd);
        if (walletModel->updateAddressBookLabels(crctAdd, label.toUtf8().constData(),
