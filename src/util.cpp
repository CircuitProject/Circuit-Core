...
const char * const CIRCUIT_CONF_FILENAME = "circuit.conf";
const char * const CIRCUIT_PID_FILENAME = "circuit.pid";
const char * const CIRCUIT_MASTERNODE_CONF_FILENAME = "masternode.conf";


// CIRCUIT only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\CIRCUIT
// Windows >= Vista: C:\Users\Username\AppData\Roaming\CIRCUIT
// Mac: ~/Library/Application Support/CIRCUIT
// Unix: ~/.circuit
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CIRCUIT";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CIRCUITParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CIRCUITParams
    // Mac: ~/Library/Application Support/CIRCUITParams
    // Unix: ~/.circuit-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CIRCUITParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "CIRCUITParams";
#else
    // Unix
    return pathRet / ".circuit-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/circuit/sapling-spend.params";
        sapling_output = "/usr/share/circuit/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/circuit/sapling-spend.params";
            sapling_output = "/usr/local/share/circuit/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", CIRCUIT_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", CIRCUIT_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override circuit.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", CIRCUIT_PID_FILENAME));
