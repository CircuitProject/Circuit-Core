...
CCrctStake* CCrctStake::NewCrctStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CCrctStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CCrctStake(txPrev.vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CCrctStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CCrctStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CCrctStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CCrctStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CCrctStake::GetUniqueness() const
{
    //The unique identifier for a CRCT stake is the outpoint
...
const CBlockIndex* CCrctStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CCrctStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CCrctStake::ContextCheck(int nHeight, uint32_t nTime)
{
