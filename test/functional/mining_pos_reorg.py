...
class ReorgStakeTest(CircuitTestFramework):
    ...
    def check_money_supply(self, expected_crct):
        # verify that nodes have the expected CRCT supply
        crct_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(crct_supply, [DecimalAmt(expected_crct)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that CRCT supply was properly updated after the reorgs
        self.log.info("Check CRCT supply...")
